...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        "🌟 *Selamat Datang di USDT Calculator Bot* 🌟\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/admin - Akses panel admin (hanya untuk admin)"
    )
    return MAIN_MENU
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Admin menu callbacks
        ...
        await query.edit_message_text(
            f"Rate Beli saat ini: {format_currency(buy_rate)}\n\n"
        ...
        await query.edit_message_text(
            f"Rate Jual saat ini: {format_currency(sell_rate)}\n\n"
...
async def handle_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📊 Set Rate Beli":
        msg = await update.message.reply_text(
            "📈 *Set Rate Beli USDT*\n\n"
            "Masukkan rate beli USDT (dalam IDR):",
            parse_mode='Markdown'
        )
        context.user_data['last_message_id'] = msg.message_id
        return SET_BUY_RATE
        ...
        msg = await update.message.reply_text(
            "📉 *Set Rate Jual USDT*\n\n"
            "Masukkan rate jual USDT (dalam IDR):",
            parse_mode='Markdown'
        )
        context.user_data['last_message_id'] = msg.message_id
        return SET_SELL_RATE
...
async def handle_buy_usdt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = await calculate_transaction_async(db_session, usdt_amount, 'buy')
        if not result:
            msg = await update.message.reply_text(
                "❌ Gagal menghitung transaksi. Silakan coba lagi:",
                reply_markup=get_back_keyboard()
            )
            context.user_data['last_message_id'] = msg.message_id
            return BUY_USDT
        ...
        msg = await update.message.reply_text(
            ...
            f"╰────────────────────\n"
            f"╭────〔 BELI USDT 〕──────\n"
            f"┊ • Jumlah : {usdt_amount:.2f} USDT\n"
            f"┊ • Rate : {format_currency(result['rate'])}\n"
            f"┊ • Fee : {format_currency(result['fee'])}\n"
            f"┊ • Total : {format_currency(result['total_amount'])}\n"
            f"╰────────────────────",
...
async def handle_sell_usdt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = await calculate_transaction_async(db_session, usdt_amount, 'sell')
        if not result:
            msg = await update.message.reply_text(
                "❌ Gagal menghitung transaksi. Silakan coba lagi:",
                reply_markup=get_back_keyboard()
...
async def handle_set_buy_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        update_rate(db_session, 'buy', rate)
        
        msg = await update.message.reply_text(
            f"✅ Rate beli USDT berhasil diupdate menjadi {format_currency(rate)}",
            reply_markup=get_admin_menu_keyboard()
        )
        context.user_data['last_message_id'] = msg.message_id
        return ADMIN_MENU
...
async def handle_set_sell_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Update rate in database
        update_rate(db_session, 'sell', rate)
        
        msg = await update.message.reply_text(
            f"✅ Rate jual USDT berhasil diupdate menjadi {format_currency(rate)}",
            reply_markup=get_admin_menu_keyboard()
        )
        context.user_data['last_message_id'] = msg.message_id
        return ADMIN_MENU
